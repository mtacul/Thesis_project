...
q0_TRIADS = [q_orbit_B[0]]
q1_TRIADS = [q_orbit_B[1]]
q2_TRIADS = [q_orbit_B[2]]
q3_TRIADS = [q_orbit_B[3]]
...
pot_magn_x = [np.random.normal(0, 0.15, 1)]
pot_magn_y = [np.random.normal(0, 0.15, 1)]
pot_magn_z = [np.random.normal(0, 0.15, 1)]
...
for i in range(len(t_aux)-1):
    ...
    w_TRIAD_PD = np.array([w0_values[-1],w1_values[-1],w2_values[-1]])
    # w_TRIAD_PD = w_in_NL - np.dot(functions.quaternion_to_dcm(q_in_PD),w0_oi)
    # w_TRIAD_PD = functions.simulate_gyros_reading(w_TRIAD_PDn, noise_rms_good*0.00001,0)
    ...
    if u_PD_NL[0]>15: 
        u_PD_NL[0] = 15
        potencia_x.append(1.11)
    else:
        u_PD_NL[0] = u_PD_NL[0]
        potencia_x.append(u_PD_NL[0]*1.11/15)

    if u_PD_NL[1]>15:
        u_PD_NL[1] = 15
        potencia_y.append(1.11)

    else:
        u_PD_NL[1] = u_PD_NL[1]
        potencia_y.append(u_PD_NL[1]*1.11/15)

    if u_PD_NL[2]>15:
        u_PD_NL[2] = 15
        potencia_z.append(1.11)
    else:
        u_PD_NL[2] = u_PD_NL[2]
        potencia_z.append(u_PD_NL[2]*1.11/15)
    ...
    w_gyros = functions.simulate_gyros_reading(w_new, 0,0)
    w0_values.append( w_gyros[0])
    w1_values.append( w_gyros[1])
    w2_values.append( w_gyros[2])
    ...
    B_magn_good = functions.simulate_magnetometer_reading(B_body, ruido)
    Bx_bodys.append(B_magn_good[0])
    By_bodys.append(B_magn_good[1])
    Bz_bodys.append(B_magn_good[2])
    ...
    vsun_good = functions.simulate_sunsensor_reading(vsun_body, sigma_good)
    vsun_bodys_x.append(vsun_good[0])
    vsun_bodys_y.append(vsun_good[1])
    vsun_bodys_z.append(vsun_good[2])
    
    delta_q = np.random.rand(4)*0.15 #np.linalg.norm(np.array([0.1,ruido,sigma_good]))
    ...
    pot_magn_x.append(np.random.normal(0, 0.15, 1))
    pot_magn_y.append(np.random.normal(0, 0.15, 1))
    pot_magn_z.append(np.random.normal(0, 0.15, 1))
...
potencia_x = np.array(potencia_x)
potencia_y = np.array(potencia_y)
potencia_z = np.array(potencia_z)

pot_magn_x = np.array(pot_magn_x)    
pot_magn_y = np.array(pot_magn_y)    
pot_magn_z = np.array(pot_magn_z)    
...
# Nombre del archivo
archivo_c = "control_good_magnet_o.csv"

# Abrir el archivo en modo escritura
with open(archivo_c, 'w') as f:
    # Escribir los encabezados
    f.write("t_aux, Roll,Pitch,Yaw, q0_rot, q1_rot, q2_rot, q3_rot, q0_TRIAD, q1_TRIAD, q2_TRIAD, q3_TRIAD, w0_values, w1_values, w2_values, potencia_x, potencia_y, potencia_z,pot_magn_x,pot_magn_y,pot_magn_z _z\n")
    ...
    for i in range(len(t_aux)):
        f.write("{}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}\n".format(
            ...
            w0_values[i], w1_values[i], w2_values[i], potencia_x[i], potencia_y[i], potencia_z[i],
            pot_magn_x[i],pot_magn_y[i],pot_magn_z[i]
        ))
