...
for i in range(len(t_aux)-1):
    ...
    w_gyros_med = functions.simulate_gyros_reading(w_new, 0,0)
    w0_values.append(w_gyros_med[0])
    w1_values.append(w_gyros_med[1])
    w2_values.append(w_gyros_med[2])
    ...
    vsun_med = functions.simulate_sunsensor_reading(vsun_body, sigma_med)
    
    vsun_bodys_x.append(vsun_med[0])
    vsun_bodys_y.append(vsun_med[1])
    vsun_bodys_z.append(vsun_med[2])
    
    delta_q = np.random.rand(4)*0.25 #np.linalg.norm(np.array([0.1,ruido,sigma_med]))
